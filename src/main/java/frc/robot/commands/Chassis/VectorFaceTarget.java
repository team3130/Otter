...
public class VectorFaceTarget extends Command {
  ...
  public void execute() {
    ...
    if (DriverStation.getAlliance().isPresent() && DriverStation.getAlliance().get() == DriverStation.Alliance.Red) {
      yJoystick = controller.getRawAxis(Constants.PS5.LST_AXS_LJOYSTICKX);
      xJoystick = controller.getRawAxis(Constants.PS5.LST_AXS_LJOYSTICKY);
    } else { // blue alliance
      yJoystick = -controller.getRawAxis(Constants.PS5.LST_AXS_LJOYSTICKX);
      xJoystick = -controller.getRawAxis(Constants.PS5.LST_AXS_LJOYSTICKY);
    }
    ...
    thetaJoystick = -controller.getRawAxis(Constants.PS5.LST_AXS_RJOYSTICKX);

    // angle used for targeting
    omegaJoystick = -controller.getRawAxis(Constants.PS5.LST_AXS_RJOYSTICKY);
    ...
    if (chassis.isTargetingSpeaker(omegaJoystick, thetaJoystick)) {
      chassis.resetTargetSpeakerController();
      thetaJoystick = chassis.goToTargetPower();
    } else if (chassis.isTargetingBackClimb(omegaJoystick, thetaJoystick)) {
